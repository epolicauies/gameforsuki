...
// --- Secret Skip Key ---
const SECRET_SKIP_KEY = 'S';
const SECRET_SKIP_MODIFIER = 'Shift'; // Hold Shift and press S
...
// --- Secret Skip Key Event ---
document.addEventListener('keydown', function(e) {
  if(stage === 'playing' && e.key.toUpperCase() === SECRET_SKIP_KEY && e.shiftKey) {
    winGame();
  }
});
...
function drawClouds() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.arc(cloudX + 60, cloudY + i * 20, 30, 0, Math.PI * 2);
    ctx.arc(cloudX + 90, cloudY + i * 20, 20, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
